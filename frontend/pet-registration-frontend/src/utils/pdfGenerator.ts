...
function drawTextInBox({
  ...
  while (
    font.widthOfTextAtSize(text, fontSize) > width &&
...
export async function generatePdf(record: any) {
  try {
    ...
    map(record.specialNotes,     436, 842-404, 58, 404-382);         // 특이사항 (2줄 대응 가능)
    ...
    if (record.signaturePath) {
      const imageUrl = `http://localhost:3000${record.signaturePath}`;
